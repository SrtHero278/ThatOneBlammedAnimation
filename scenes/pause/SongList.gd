...
@onready var folders:PackedStringArray = DirAccess.get_directories_at("res://assets/songs")
...
func load_song(index:int):
	...
	var meta_path = "res://assets/songs/%s/meta.json" % folder
	if not FileAccess.file_exists(meta_path): return
